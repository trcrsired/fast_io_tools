...
struct common_arg
{
	void (*function_ptr)(output out,common_arg<output>* ptr);
	void *arg_ptr;
	std::size_t arg_len;
};

template<output_stream output>
inline void print_freestanding_decay_no_status_common(output out,common_arg<output>* first,common_arg<output>* last)
{
	for(;first!=last;++first)
	{
		first->function_ptr(out,first);
	}
}

template<typename output,bool line,typename T>
inline void print_freestanding_common_arg_dispatch_single(output out,common_arg<output>* ptr)
{
	using char_type = typename output::char_type;
	if constexpr(scatter_printable<char_type,T>)
	{
		basic_io_scatter_t<char_type> scatter{reinterpret_cast<char_type const*>(ptr->arg_ptr),ptr->arg_len};
		::fast_io::details::decay::print_control<line>(out,scatter);
	}
	else if constexpr(sizeof(T)<=sizeof(void*))
	{
		std::byte tbuffer[sizeof(T)];
		if constexpr(sizeof(T)!=0)
		{
			__builtin_memcpy(tbuffer,__builtin_addressof(ptr->arg_ptr),sizeof(T));
		}
		T t{__builtin_bit_cast(T,tbuffer)};
		::fast_io::details::decay::print_control<line>(out,t);
	}
	else
	{
		::fast_io::details::decay::print_control<line>(out,*reinterpret_cast<T*>(ptr->arg_ptr));
	}
}
...
[[msvc::forceinline]]
#endif
inline void print_freestanding_common_arg_dispatch(common_arg<output>* arg,T&& t,Args&&... args)
{
	using char_type = typename output::char_type;
	constexpr bool need_to_do_with_line{sizeof...(Args)==0?line:false};
	arg->function_ptr=print_freestanding_common_arg_dispatch_single<output,need_to_do_with_line,T>;
	if constexpr(scatter_printable<char_type,T>)
	{
		auto scatter=print_scatter_define(io_reserve_type<char_type,T>,t);
		arg->arg_ptr=const_cast<void*>(reinterpret_cast<void const*>(scatter.base));
		arg->arg_len=scatter.len;
	}
	else if constexpr(sizeof(T)<=sizeof(void*))
	{
		void* ptr;
		...
		{
			__builtin_memcpy(__builtin_addressof(ptr),__builtin_addressof(t),sizeof(T));
		}
		arg->arg_ptr=ptr;
	}
	else
	{
		arg->arg_ptr=__builtin_addressof(t);
	}
	if constexpr(sizeof...(Args)!=0)
	{
		print_freestanding_common_arg_dispatch<line,output>(arg+1,::fast_io::freestanding::forward<Args>(args)...);
	}
}
...
true
			...
			else
			{
				...
				{
					constexpr std::size_t args_num{sizeof...(Args)};
					common_arg<output> args_buffer[args_num];
					print_freestanding_common_arg_dispatch<line,output>(args_buffer,::fast_io::freestanding::forward<Args>(args)...);
					print_freestanding_decay_no_status_common(out,args_buffer,args_buffer+args_num);
				}
			}
		}
	}
