...
inline constexpr std::uint32_t calculate_crc32(std::uint32_t crc,std::byte const* block_start,std::size_t block_bytes) noexcept
{
	...
	{
		if constexpr(crc32c)
			crc=crc32c_tb[(static_cast<std::uint32_t>(*i)^crc)&0xff]^(crc>>8);
		else
			crc=crc32_tb[(static_cast<std::uint32_t>(*i)^crc)&0xff]^(crc>>8);
	}
	return crc;
}
