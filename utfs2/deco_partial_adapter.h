				...
				output_char_type *tofirst,output_char_type *tolast)
	{
		...
		if(fromdiff<diff)
		{
			if(fromdiff!=0)
			{
				...
				::fast_io::details::non_overlapped_copy_n(fromfirst,fromdiff,remainedit);
			
				auto [fromit,toit]=deco.process_chars(remained_buffer,remainedit,tofirst,tolast);
				tofirst=toit;
				if(fromit==remainedit)
				{
					fromfirst+=fromdiff;
					remained=0;
				}
				...
				{
					auto rmdiff{remainedit-fromit};

					::fast_io::freestanding::copy_n();
					if(remained<rmdiff)
					{
						fromfirst+=rmdiff-remained;
					}
				}
			}
