...
inline constexpr void iobuf_read_unhappy_tie_impl(T& bios,Iter first,Iter last)
{
...
inline constexpr Iter iobuf_read_unhappy_impl(T& bios,Iter first,Iter last)
{
	...
	{
		iobuf_read_unhappy_tie_impl(bios,first,last);
	}
...
[[gnu::cold]]
#endif
inline constexpr Iter iobuf_read_all_unhappy_impl(T& bios,Iter first,Iter last)
{
	if constexpr(((T::mode&buffer_mode::out)==buffer_mode::out)&&((T::mode&buffer_mode::tie)==buffer_mode::tie))
	{
		if constexpr(details::has_external_decorator_impl<typename T::decorators_type>)
...
std::size_t bfs,::fast_io::freestanding::random_access_iterator Iter>
requires (((mde&buffer_mode::in)==buffer_mode::in)&&details::allow_iobuf_punning<typename decorators::internal_type,Iter>)
inline constexpr void read_all_define(basic_io_buffer<handletype,mde,decorators,bfs>& bios,Iter first,Iter last)
{
	using iter_char_type = ::fast_io::freestanding::iter_value_t<Iter>;
	using char_type = typename decorators::internal_type;
	if constexpr(std::same_as<iter_char_type,char_type>)
	{
		if constexpr(::fast_io::freestanding::contiguous_iterator<Iter>&&!std::is_pointer_v<Iter>)
		{
			read_all_define(bios,::fast_io::freestanding::to_address(first),::fast_io::freestanding::to_address(last));
		}
		else
		{
			std::size_t diff(static_cast<std::size_t>(last-first));
			std::size_t remain_space(bios.ibuffer.buffer_end-bios.ibuffer.buffer_curr);
			if(remain_space<diff)[[unlikely]]
				details::iobuf_read_all_unhappy_impl(bios,first,last);
			first=details::non_overlapped_copy_n(bios.ibuffer.buffer_curr,diff,first);
			bios.ibuffer.buffer_curr+=diff;
		}
	}
	...
	{
		read_all_define(bios,reinterpret_cast<char*>(::fast_io::freestanding::to_address(first)),reinterpret_cast<char*>(::fast_io::freestanding::to_address(last)));
	}
}
