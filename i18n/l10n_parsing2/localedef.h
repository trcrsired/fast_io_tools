...
using scatter_size_type = std::uint_least32_t;

template<typename T>
struct basic_scatter {
	inline static std::vector<T> buffer;
	[[nodiscard]] inline static scatter_size_type buffer_size() noexcept {
		return static_cast<scatter_size_type>(buffer.size());
	}
	scatter_size_type rva;
	scatter_size_type length;
};
