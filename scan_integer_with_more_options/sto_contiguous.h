...
inline constexpr prefix_parse_result<Iter> scan_int_contiguous_prefix_define_impl(Iter first,Iter last) noexcept
{
	using char_type = ::fast_io::freestanding::iter_value_t<Iter>;
//	std::size_t diff{static_cast<std::size_t>(last-first)};
	if constexpr(base==8)
	{//oct is a historical mistake
		if(first==last)
		{
			return {first,false};
		}
		...
		if(first==last)
		{
			return {first,(*first==char_literal_v<u8'0',char_type>)};
		}
		++first;
		return {first,parse_code::ok};
	}
	else if constexpr(base==16||base==2||base==3)
	{
		std::size_t diff{static_cast<std::size_t>(last-first)};
		if(diff<3)
			return {first,false};
		bool zero_ch{*first==char_literal_v<u8'0',char_type>};
		++first;
		auto next_ch{*first};
		++first;
		if constexpr(base==16)
		{
			return {first,((zero_ch)&&
					(next_ch==char_literal_v<u8'X',char_type>||next_ch==char_literal_v<u8'x',char_type>))};
		}
		else if constexpr(base==2)
		{
			return {first,((zero_ch)&&
					(next_ch==char_literal_v<u8'B',char_type>||next_ch==char_literal_v<u8'b',char_type>))};
		}
		else
		{
			return {first,((zero_ch)&&
					(next_ch==char_literal_v<u8'T',char_type>||next_ch==char_literal_v<u8't',char_type>))};
		}
	}
	else
	{
		...
		if constexpr(base<10)
		{
			if(diff<5)
				return {first,false};
			constexpr char_type ascii_base_digit{char_literal_v<static_cast<char8_t>(u8'0'+static_cast<char8_t>(base)),char_type>};
...
bool prefix,
bool showbase,
bool ignore_seperator,
bool showpos,
bool noskipws,
bool skip_all_none_domain_characters,
bool skip_zero,
...
inline constexpr parse_result<Iter> scan_int_contiguous_define_impl(Iter first,Iter last,T& t) noexcept
{
	...
	{
		if constexpr(skip_all_none_domain_characters)
		{
			for(;first!=last;++first)
			{
				if(::fast_io::details::char_is_digit<base>(static_cast<unsigned_char_type>(*first)))
				{
					break;
				}
				if constexpr(showpos)
				{
					if(*first==char_literal_v<u8'+',char_type>)
					{
						...
						{
							++first;
							if(first==last||*first==char_literal_v<u8'+',char_type>||char_literal_v<u8'-',char_type>)
								return {first,parse_code::invalid};
						}
						break;
					}
...
template<std::size_t bs,::fast_io::manipulators::scan_flags flags>
inline constexpr ::fast_io::manipulators::scalar_flags base_scan_mani_flags_cache{.base=bs,
	.showbase=((flags&::fast_io::manipulators::scan_flags::prefix)==::fast_io::manipulators::scan_flags::prefix),
	.showpos=((flags&::fast_io::manipulators::scan_flags::allow_plus_sign)==::fast_io::manipulators::scan_flags::allow_plus_sign),
	.noskipws=((flags&::fast_io::manipulators::scan_flags::noskipws)==::fast_io::manipulators::scan_flags::noskipws),
	.floating=::fast_io::manipulators::floating_format::fixed,
	.ignore_seperator=((flags&::fast_io::manipulators::scan_flags::ignore_seperator)==::fast_io::manipulators::scan_flags::ignore_seperator),
	.exact=((flags&::fast_io::manipulators::scan_flags::exact)==::fast_io::manipulators::scan_flags::exact),
	.skip_all_none_domain_characters=((flags&::fast_io::manipulators::scan_flags::skip_all_none_domain_characters)==::fast_io::manipulators::scan_flags::skip_all_none_domain_characters),
	.skip_zero=((flags&::fast_io::manipulators::scan_flags::skip_zero)==::fast_io::manipulators::scan_flags::skip_zero)};
}
...
template<std::size_t bs,scan_flags flags=scan_flags::none,::fast_io::details::my_integral scalar_type>
inline constexpr scalar_manip_t<::fast_io::details::base_scan_mani_flags_cache<bs,flags>,scalar_type&> base_get(scalar_type& t) noexcept
{
	return {t};
}

template<scan_flags flags=scan_flags::none,::fast_io::details::my_integral scalar_type>
inline constexpr scalar_manip_t<::fast_io::details::base_scan_mani_flags_cache<2,flags>,scalar_type&> bin_get(scalar_type& t) noexcept
{
	return base_get<2,flags>(t);
}

template<scan_flags flags=scan_flags::none,::fast_io::details::my_integral scalar_type>
inline constexpr scalar_manip_t<::fast_io::details::base_scan_mani_flags_cache<10,flags>,scalar_type&> dec_get(scalar_type& t) noexcept
{
	return base_get<10,flags>(t);
}

template<scan_flags flags=scan_flags::none,::fast_io::details::my_integral scalar_type>
inline constexpr scalar_manip_t<::fast_io::details::base_scan_mani_flags_cache<16,flags>,scalar_type&> hex_get(scalar_type& t) noexcept
{
	return base_get<16,flags>(t);
}

template<scan_flags flags=scan_flags::none,::fast_io::details::my_integral scalar_type>
inline constexpr scalar_manip_t<::fast_io::details::base_scan_mani_flags_cache<8,flags>,scalar_type&> oct_get(scalar_type& t) noexcept
{
	return base_get<8,flags>(t);
}

}

template<details::my_integral T>
inline constexpr ::fast_io::manipulators::scalar_manip_t<::fast_io::details::base_scan_mani_flags_cache<10,::fast_io::manipulators::scan_flags::none>,T&> scan_alias_define(io_alias_t,T& t) noexcept
{
	return {t};
}
